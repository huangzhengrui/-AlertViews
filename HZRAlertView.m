...
#define Image_W         105.0f
#define Image_H         125.0f
...
-(instancetype)initWithTitle:(NSString *)title Image:(UIImage *)image message:(NSString *)message cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(HZRAlertClickIndexBlock)block{
    if(self=[super init]){
        ...
        if (image) {
            _imageView=[[UIImageView alloc] initWithFrame:CGRectMake(AlertView_W/2-Image_W/2, _titleLab.frame.origin.y+_titleLab.frame.size.height+10, Image_W, Image_H)];
        ...
        if (message.length>0) {
            ...
            if (image) {
                _messageLab.frame=CGRectMake(messageLabSpace, _imageView.frame.size.height+_imageView.frame.origin.y+10, AlertView_W-messageLabSpace*2, endMessageLabH);
            }else{
        ...
        //计算_alertView的高度
        _alertView.frame=CGRectMake(0, 0, AlertView_W, _messageLab.frame.size.height+_titleLab.frame.size.height+LXABtn_H+40+_imageView.frame.size.height);
...
-(void)setShowAnimation{
    ...
    switch (_animationStyle) {
        ...
        case HZRASAnimationDefault://小到放大
        {
...
-(void)setAnimationStyle:(HZRAShowAnimationStyle)animationStyle{
